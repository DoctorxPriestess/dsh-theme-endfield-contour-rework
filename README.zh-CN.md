...
- 等高线背景、等高线滚动开关、滚动方向（8 个方向）、滚动速度（`12 / 24 / 48 / 96 / 192` px/s）、等高线疏密度（稀疏 / 适中 / 密集 / 极密）、地形粗糙度（**滑块，12 档：平原 → 丘陵 → 山地，逐档叠加高原 / 悬崖 / 山脊线 / 深谷 / 湖海与地貌斑块**）；
- 背景水印，以及水印保持显示；
- 启动加载动画；
- 雷霆大字，以及大字入场动画。
...
- 改方向或速度只更新两个缓存数值；改疏密度则对**同一个地形**重新提取一次；改**地形粗糙度**会用同一个 seed 重新生成地形（低档平坦如平原、高档崎岖如高山；第 6 档起出现高原并随档位逐减，第 8 档起出现悬崖并在第 12 档最多；地貌强度往上按越来越快的凸曲线抬升，第 12 档特征地形约占 61%，每种特征地形占比都 < 75%）；
