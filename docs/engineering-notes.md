...
选择这 12 组数值靠的是**把整条阶梯渲染出来对比**（切出引擎、逐档建场 + 提取 + 栅格化）。两个实测约束决定了最终数值：(1) **可读性**——最细倍频的振幅占比超过约 1/10 且其单元尺度接近 10px 采样网格时，画面从「地形」退化成均匀噪点，因此 persistence 上限取 0.62，且高档位刻意把基格留在 250px 以上，好让阶梯保住第 5 个倍频、把细部能量摊到两个尺度上而不是堆在最细那层；(2) **档位可辨**——基格会按纹理被量化成整数格距，相邻两档的四舍五入结果一旦相同，两档画出来就一模一样（实测最初的低档里 820 与 730 都落在 5 格，100 多条等高线里只差 2 条），因此低半段刻意走过互不相同的格距，高半段格距饱和后由 persistence 单独承担。这两条约束都写进了 `test/contour-roughness.test.js` 的断言（单调性、逐档步进 ≥4%、最细倍频占比 ≤12%）。
