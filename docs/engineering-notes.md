...
**地形生成。** 经典梯度噪声（种子派生的 512 项置换表 + 8 个单位梯度方向），5 个倍频（fBm），persistence `0.5`，在 10px 网格上采样成 `Float32Array`。**周期即纹理尺寸**：每个倍频的周期取 `round(尺寸 / 基格)` 并钳制到置换表安全上限，于是 `(x / tw) * period` 在 `x = tw` 时恰好落在整数周期上——纹理左右/上下**精确**连续，滚动没有接缝，也不需要靠「把周期做很长」来掩盖。
...
改动方向或速度只更新两个缓存数值；改密度对**同一个场**重新提取一次；resize 只在量化后的目标尺寸真正变化时重建（200ms 防抖）。地形随机性只由置换表承载，所以「同 seed + 同尺寸 → 完全相同的地形」可验证。
