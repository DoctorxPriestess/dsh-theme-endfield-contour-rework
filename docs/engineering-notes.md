...
选择这 12 组数值靠的是**把整条阶梯渲染出来对比**（切出引擎、逐档建场 + 提取 + 栅格化）。两个实测约束决定了最终数值：(1) **可读性**——最细倍频的振幅占比超过约 1/10 且其单元尺度接近 10px 采样网格时，画面从「地形」退化成均匀噪点，因此 persistence 上限取 0.62，且高档位刻意把基格留在 250px 以上，好让阶梯保住第 5 个倍频、把细部能量摊到两个尺度上而不是堆在最细那层；(2) **档位可辨**——基格会按纹理被量化成整数格距，相邻两档的四舍五入结果一旦相同，两档画出来就一模一样（实测最初的低档里 820 与 730 都落在 5 格，100 多条等高线里只差 2 条），因此低半段刻意走过互不相同的格距，高半段格距饱和后由 persistence 单独承担。这两条约束都写进了 `test/contour-roughness.test.js` 的断言（单调性、最细倍频占比 ≤12%、高原占比与悬崖占比逐档递增）。

**悬崖与高原：软阶梯（2026-09 追加）。** 第 10–12 档在地形上再叠一道**软阶梯**（`contourTerrace`，强度表 `CONTOUR_ROUGHNESS_TERRACE`）。它是把建好的 fBm 场按自身 `[min,max]` 做逐点的高度重映射：`steps = 2 + round(4 × 强度)` 级，每级里 `1 - 2w` 的比例是**真正水平**的平台，平台之间用 `w = min(0.45, 0.5 × 强度)` 宽的 smoothstep 斜坡连接。落到图上就是高原（平台内部等值线穿不过去 → 空白）加悬崖（斜坡在几十像素里跨好几条等值线 → 线束）。

为什么是**软边**而不是 `floor()` 硬量化：硬量化会往高度场里塞进真正的坡度不连续，跨过它的等值线必然带尖角，而「全图画不出一个 >8° 的转角」是既有的硬约束，靠 Chaikin/B 样条事后抹平是抹不掉的（这正是当初 cusp 套件存在的原因）。smoothstep 软边只有「陡」没有「断」，所以 `test/contour-cusps.test.js` 现在**把每个带阶梯的档位单独扫一遍**（1152×648 与 320×240 两个尺寸 × 两个密度），实测最大转角仍是 1.1°，与不带阶梯的地形完全一致。阶梯逐点作用在成品场上，因此可平铺性（映射不碰坐标，边界处的 `(x/tw)*period` 逻辑不变）与「同 seed 同尺寸 → 同地形」的确定性都不受影响；强度为 0 的档位走的是同一条 `if` 的短路分支，场与改动前**逐位相同**。

阶梯的形状常量也是**渲染成条带逐版对比**选的（脚本思路：按真实比例把 1280×340 的条带画出来，量「高原占比 / 悬崖占比 / 每簇线束的条数」）。候选里 `3 + round(7 × 强度)`（4→10 级）纸面上更「层次分明」，实测却把一屏里三分之一以上推成空白、坡面上每条只剩一根线，读起来像图坏了；`1 + round(4 × 强度)` 又反过来把半个画面压平（高原占比 38/51/45%）。最终取 `2 + round(4 × 强度)`（4/5/6 级）：第 10 档高原约 32%、第 11 档 54%、第 12 档 68%，同时「非高原部分里属于陡坡的比例」从默认档的 35% 升到 51% → 64% → 83%。

**度量方式值得记一笔**：带阶梯的档位之间**总笔画长度几乎不变**（实测 575k → 569k px），因为阶梯把许多零散短环换成了少数长线束——用「笔画随档位增加」来判断这一档是否生效会得出错误结论。所以测试对这一段的断言换成了两个**场本身的绝对阈值**指标：相邻网格点高度差 < 0.15%×range 的算高原、> 3%×range 的算悬崖坡面；「悬崖占比」定义为 `steep / (1 - flat)`（在**剩余起伏**里的占比，而不是全图占比），否则高原一多、陡坡的绝对数量反而下降，指标会朝着效果的反方向走（实测全图占比 33.6% → 26.9%，归一化后 35% → 83%）。这两个阈值是 range 的固定比例而非分位数——分位数会随之移动，恰好会把要测的效果自己抵消掉。
