...
```bash
node test/contour-cusps.test.js       # 几何：尖点 / 重复提取 / 顶点跳变 / 碎屑 / seed 生命周期，逐档覆盖全部地貌（Node）
...
**`contour-roughness.test.js`** 守的是「唯一会改变地形形状的那个设置」。它不检查有没有抛异常——地形调错了不会抛异常，只会画出**坏地图**：某一档什么都不画、最粗糙那档糊成均匀噪点、或者地貌层悄悄盖满整张图。所以这个脚本按用户看到的方式量**整条阶梯**：

- **表**：12 张地貌表（地形类型 / 宏观高程 / 坡面形态 / 脊线 / 谷线 / 高原 / 悬崖 / 悬崖带宽 / 尖锐山峰 / 水位 / 斑块密度 / 斑块尺度）各有 12 档、取值在声明范围内；fBm 三张表的最细倍频在 5 种纹理尺寸下都不越过置乱表上限，且最细倍频振幅占比 ≤12%（实测最差 7.2%）；
- **出厂档**：第 8 档的三参数**逐字等于**出厂常量 `280 / 0.5 / 5`；`contourTerrainProfile()` 解析出这一行；
- **高原随粗糙度递减**：表值上第 6 档出现、逐档下降到第 9 档、第 10 档起为 0；**并且在场上实测**：25.8% → 22.0% → 21.5% → 15.1% → 之后无高原；
- **悬崖随粗糙度递增且第 12 档最多**：第 8 档起步、强度与带宽同时递增，场上实测 11.6% → 15.7% → 19.2% → 20.6% → **27.0%**；
- **特征地形占比 <75%（逐档断言）**：格子只要被地貌层移动 ≥2%×range（约一个等高距）就计入，并按最强的那一项归类。实测最粗档 44.0%，各档都在 4%–44% 之间；每条上限都是**生成器自己的账本**（`featAmt` / `featKind`），不是事后猜测；
- **水面是精确平面**：所有被水位钳过的格子高度必须**完全相等**（实测 8 个档位有水面，误差 0）；
- **接缝精确**：首列 == 末列、首行 == 末行（实测误差**恰好 0**），这是滚动无缝的前提；
- **可读性**：每级折线数 ≤ 90（实测最挤的第 8 档 59）；每档都出图、没有一档画出碎屑、每档构建 < 400 ms（实测最坏 64 ms）；
- **增长是凸的（越往上越快）**：第 6 档起的 7 张增长表（陡崖/带宽/斑块密度/斑块尺度/脊线/谷线/坡面形态）要求**增量非递减**（二阶差分 ≥ 0），并跳过「关闭 → 首次开启」那一步（陡崖在第 8 档才打开，那一跳是功能启用而不是增长步）。这条把「每档加一样的量」的线性增长判为失败。低档（第 1–5 档）刻意不在范围内：它们的地貌预算本来就是不动的。
- **最粗档的特征占比有明确窗口 55%–65%**（1.3.1 实测 44.0% → 现在 61.4%），按实测场断言；用旧带宽跑一遍确认会报红（`48.5% ... outside the stated 55.0%-65.0% window`）。
- **高档不会反而变平**：高山段（9–12 档）任何一档的**实测笔画长度**不得低于左右邻档均值的 95%，且最粗的档位必须是第 12 档（实测 467k → 473k → 477k → 503k）。这条是为一个真实缺陷补的：第 10 档曾因一个**独有的盆地偏向**（宏观高程 −0.16，而 11/12 是 +0.20/+0.26）让笔画掉到 429k——两侧是 467k/477k——滑块往上滑画面反而更平静，而当时**每张参数表都合规、整套测试全绿**。这类缺陷按定义抓不到参数，只能量场；断言用旧值跑过确认会报红。

**`contour-cusps.test.js` 现在把每个带地貌的档位单独扫一遍**（1152×648 与 320×240 × 两个密度 × 全部 12 档），因为地貌层（阶梯、峰谷重映射、斑块边缘）是唯一有可能往高度场里塞进真尖角的改动。实测各档最大转角 ≤ 1.1°，与改动前完全一致；「≥8° 转角」「顶点跳变」「碎屑阈值」等既有断言对这一段同样生效。
